...
class RerankingEvaluator(Evaluator):
    ...
    def compute_metrics_batched(self, model: Encoder | EncoderWithQueryCorpusEncode):
        ...
        if isinstance(self.samples[0]["query"], str):
            # np.asarray
            all_query_embs, default, moe_rw = encode_queries_func(
                    [sample["query"] for sample in self.samples],
                    prompt_name=self.task_name,
                    **self.encode_kwargs,
                )
            all_query_embs = np.asarray(all_query_embs)
            default = np.asarray(default)
            moe_rw = np.asarray(moe_rw)
    ...
    def _encode_candidates(
        self, encode_corpus_func, batched, all_query_embs=None, encode_queries_func=None, default=None, moe_rw=None,
    ):
        all_mrr_scores = []
        all_ap_scores = []
        all_conf_scores = []
    ...
    def _encode_candidates_batched(
        ...
        default1, moe_rw1 = default, moe_rw
        all_docs = []
        for sample in self.samples:
            all_docs.extend(sample["positive"])
            all_docs.extend(sample["negative"])

        all_docs_embs, default2, moe_rw2 = self._encode_unique_texts(
            all_docs,
            encode_corpus_func,
            prompt_name=self.task_name,
            **self.encode_kwargs,
        ...
        if self.encode_kwargs['do_pca']:
            combined_enc = np.concatenate([all_query_embs, all_docs_embs], axis=0)
            pca = PCA(n_components=min(self.encode_kwargs['pca_dim'], combined_enc.shape[0]))
            combined_enc = pca.fit_transform(combined_enc)
            
            all_query_embs = combined_enc[:all_query_embs.shape[0], :]
            all_docs_embs = combined_enc[all_query_embs.shape[0]:, :]
            
            combined_enc = np.concatenate([default1, default2], axis=0)
            pca = PCA(n_components=min(self.encode_kwargs['pca_dim'], combined_enc.shape[0]))
            combined_enc = pca.fit_transform(combined_enc)
            
            default1 = combined_enc[:default1.shape[0], :]
            default2 = combined_enc[default1.shape[0]:, :]
            
            combined_enc = np.concatenate([moe_rw1, moe_rw2], axis=0)
            pca = PCA(n_components=min(self.encode_kwargs['pca_dim'], combined_enc.shape[0]))
            combined_enc = pca.fit_transform(combined_enc)
            
            moe_rw1 = combined_enc[:moe_rw1.shape[0], :]
            moe_rw2 = combined_enc[moe_rw1.shape[0]:, :]
        ...
        for instance in self.samples:
            ...
            default1_ = default1[query_idx : query_idx + num_subqueries]
            moe_rw1_ = moe_rw1[query_idx : query_idx + num_subqueries]
            ...
            default2_ = default2[docs_idx : docs_idx + num_pos + num_neg]
            moe_rw2_ = moe_rw2[docs_idx : docs_idx + num_pos + num_neg]
    ...
    def _apply_sim_scores(
        ...
        if self.encode_kwargs['similarity_ensemble']:
            main_scores = self._compute_sim_scores_instance(default1, default2)
            aux_scores = self._compute_sim_scores_instance(moe_rw1, moe_rw2)
            scores = self._compute_metrics_instance(sim_scores, is_relevant, main_scores, aux_scores)
        else:
    ...
    def _encode_unique_texts(
        ...
        **encode_kwargs: Any,
        ...
        all_unique_texts_embs, default, moe_rw = encode_fn(all_unique_texts, prompt_name=prompt_name, **encode_kwargs)
        all_unique_texts_embs = np.asarray(all_unique_texts_embs)
        default = np.asarray(default)
        moe_rw = np.asarray(moe_rw)
        return all_unique_texts_embs[all_texts_indexes], default[all_texts_indexes], moe_rw[all_texts_indexes]
    ...
    def _compute_metrics_instance(
        self, sim_scores: torch.Tensor, is_relevant: list[bool], default_scores: torch.Tensor=None, aux_scores: torch.Tensor=None
    ) -> dict[str, float]:
        ...
        if self.encode_kwargs['similarity_ensemble']:
            ap = self.ap_score(is_relevant, [default_scores[i] + aux_scores[i] * self.encode_kwargs['similarity_weights'] for i in range(default_scores.shape[0])])
        else:
