    ...
    def search(
        ...
        **kwargs,
        ...
        if isinstance(queries[0], list):  # type: ignore
            ...
            query_embeddings, _, _ = self.model.encode_queries(
                queries,  # type: ignore
                prompt_name=prompt_name,
                **self.encode_kwargs,
            )
        ...
        for batch_num, corpus_start_idx in enumerate(itr):
            ...
            if (
                ...
                # Encode chunk of corpus
                sub_corpus_embeddings, _, _ = self.model.encode_corpus(
            ...
            if self.encode_kwargs['do_pca']:
                combined_enc = np.concatenate([query_embeddings, sub_corpus_embeddings], axis=0)
                pca = PCA(n_components=min(self.encode_kwargs['pca_dim'], combined_enc.shape[0]))
                combined_enc = pca.fit_transform(combined_enc)
                
                query_embeddings = combined_enc[:query_embeddings.shape[0], :]
                sub_corpus_embeddings = combined_enc[query_embeddings.shape[0]:, :]
