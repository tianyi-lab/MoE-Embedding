...
class PairClassificationEvaluator(Evaluator):
    ...
    def compute_metrics(
        ...
        *,
        ...
        embeddings, default, moe_rw = model_encode(
            sentences,
            model=model,
        ...
        if encode_kwargs['do_pca']:
            pca = PCA(n_components=min(encode_kwargs['pca_dim'], embeddings.shape[0]))
            embeddings = pca.fit_transform(embeddings)
            
            pca = PCA(n_components=min(encode_kwargs['pca_dim'], default.shape[0]))
            default = pca.fit_transform(default)
            
            pca = PCA(n_components=min(encode_kwargs['pca_dim'], moe_rw.shape[0]))
            moe_rw = pca.fit_transform(moe_rw)
            
        emb_dict = dict(zip(sentences, embeddings))
        default_dict = dict(zip(sentences, default))
        moe_rw_dict = dict(zip(sentences, moe_rw))
        
        embeddings1 = [emb_dict[sent] for sent in self.sentences1]
        embeddings2 = [emb_dict[sent] for sent in self.sentences2]
        
        default1 = [default_dict[sent] for sent in self.sentences1]
        default2 = [default_dict[sent] for sent in self.sentences2]
        
        moe_rw1 = [moe_rw_dict[sent] for sent in self.sentences1]
        moe_rw2 = [moe_rw_dict[sent] for sent in self.sentences2]
        ...
        if encode_kwargs['similarity_ensemble']:
            ...
            manhattan_distances_main = paired_manhattan_distances(default1, default2)
            manhattan_distances_aux = paired_manhattan_distances(moe_rw1, moe_rw2)
            
            euclidean_distances_main = paired_euclidean_distances(default1, default2)
            euclidean_distances_aux = paired_euclidean_distances(moe_rw1, moe_rw2)
            
            cosine_scores = cosine_scores_main + encode_kwargs['similarity_weights'] * cosine_scores_aux
            manhattan_distances = manhattan_distances_main + encode_kwargs['similarity_weights'] * manhattan_distances_aux
            euclidean_distances = euclidean_distances_main + encode_kwargs['similarity_weights'] * euclidean_distances_aux
        else:
