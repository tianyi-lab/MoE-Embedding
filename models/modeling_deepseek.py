...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # sent_emb = scores.view(bsz, seq_len, -1).mean(1) # average 
        # sent_emb = scores.view(bsz, seq_len, -1)[:, -1, :] # prompteol
        # print("scores", scores.shape, scores[0]) # [bsz, 64]
        # print('topk weights', topk_weight)
        sent_emb = scores.view(bsz, seq_len, -1)
        ...
        if self.training and self.alpha > 0.0:
            ...
            aux_loss = None
        return topk_idx, topk_weight, aux_loss, sent_emb
...
class DeepseekMoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        topk_idx, topk_weight, aux_loss, sent_emb = self.gate(hidden_states)
        # print('forward in DeepseekMoE', hidden_states.shape, sent_emb.shape)
...
class DeepseekDecoderLayer(nn.Module):
    def __init__(self, config: DeepseekConfig, layer_idx: int):
        ...
        # print(config.n_routed_experts)
        # print(layer_idx, config.first_k_dense_replace)
        # print(config.moe_layer_freq)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if isinstance(self.mlp, DeepseekMoE):
            hidden_states, sent_emb = self.mlp(hidden_states)
        else:
...
class SentenceDataset(Dataset):
    def __init__(self, sentences, prompt_templates):
    ...
    def __getitem__(self, idx):
        ...
        prompts = [prompt.replace('*sent 0*', sent).replace('_', ' ').strip() for prompt in self.prompt_templates]
        return prompts
...
class DeepseekForCausalLM(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        # print('in causal lm, in shape', input_ids.shape if input_ids is not None else None)
    ...
    def encode(
        self, sentences: List[str], batch_size: int = 8, **kwargs: Any
    ) -> torch.Tensor:
        """Encodes the given sentences using the encoder.

        Args:
            sentences: The sentences to encode.
            batch_size: The number of sentences to process in one batch.
        ...
        with torch.no_grad():
            embed_method = kwargs.get("embed_method", None)
            do_pca = kwargs.get("do_pca", None)
            pca_dim = kwargs.get("pca_dim", None)
            tokenizer = kwargs.get("tokenizer", None)
            emb_info = kwargs.get("emb_info", None)
            
            if embed_method == 'prompteol':
                prompt_templates = ["This sentence : \"*sent 0*\" means in one word:\""]
            elif embed_method == 'none':
                prompt_templates = ["\"*sent 0*\""]

            # Prepare dataset and dataloader
            dataset = SentenceDataset(sentences, prompt_templates)
            ...
            for batch in dataloader:
                ...
                with torch.no_grad():
                    ...
                    outputs, embeddings = self(**inputs, output_hidden_states=True, return_dict=True)
                    embeddings = embeddings.cpu()
                    
                    lst_token_emb = outputs.hidden_states[-1][:, -1, :].cpu()
                    # embeddings = torch.cat([lst_token_emb, embeddings], dim=1)
            ...
            if do_pca:
                ...
                std = all_embs.std(dim=0, keepdim=True)
                all_embs = (all_embs - mean) / (std + 1e-8)
                
                # try:
                pca = PCA(n_components=pca_dim)
                all_embs = pca.fit_transform(all_embs)
                # except:
                #     pca = PCA(n_components=all_embs.shape[0])
                #     all_embs = pca.fit_transform(all_embs)
