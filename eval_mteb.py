...
def get_args():
    ...
    parser.add_argument('--use_4bit', action='store_true', help='Use 4-bit precision.')
    parser.add_argument('--do_pca', action='store_true', help='Perform PCA on embeddings.')
    parser.add_argument('--pca_dim', type=int, default=-1)
    parser.add_argument('--emb_info', type=str, default='HS')  
    parser.add_argument('--task_idx', type=int, default=-1)
    
    parser.add_argument('--similarity_ensemble', action='store_true', help='Perform ensemble on similarity scores.')
...
replace_dict = {('SummEvalSummarization.v2', 'Summarization'): ('SummEval', 'Summarization')}
...
if __name__ == '__main__':
    ...
    if any([x in args.base_model for x in ["instructor"]]):
        assert kwargs["pooling_method"] == "mean"
    elif any([x in args.base_model for x in ["bge"]]):
    ...
    with torch.no_grad():
        model = MOEE(model_name_or_path = args.base_model, **kwargs)
        ...
        for (task_name, task_type) in tasks:
            ...
            if not args.no_instruction:
                if task_name.startswith("CQADupstack") and \
                    ...
                    print('task_name not in SET_TO_TASK_TO_DS_TO_PROMPT[args.instruction_set][task_type]: ', task_name not in SET_TO_TASK_TO_DS_TO_PROMPT[args.instruction_set][task_type])
                    
                    if task_name not in SET_TO_TASK_TO_DS_TO_PROMPT[args.instruction_set][task_type]:
                        if (task_name, task_type) in replace_dict:
                            true_task_name = replace_dict[(task_name, task_type)][0]
                        if true_task_name not in SET_TO_TASK_TO_DS_TO_PROMPT[args.instruction_set][task_type]:
                            print('Skipping task: ' + task_name)
                            continue
